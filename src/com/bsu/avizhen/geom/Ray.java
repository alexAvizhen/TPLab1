...
public class Ray extends Segment {
	...
	public Ray(Point2D firstPoint, Point2D secondPoint){
	...
	private void setPoints(Point2D firstPoint, Point2D secondPoint){
		...
		int x = (int) Math.signum(x2 - x1) * MAX_NUMBER_OF_PIXELS;
		int y = (x2 * y1 - x1 * y2 - (y1 - y2) * x) / (x2 - x1);
		setSecondPoint(new Point(x, y));
	}
}//end com.bsu.avizhen.geom.Ray